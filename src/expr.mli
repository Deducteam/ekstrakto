...
(* [type_app t args] checks that type t can be applied to the given
   arguments, and reutrns the return type.  @raise Type_Mismatch if
   the type and arguments do not match.  *)

(* Misc functions *)
val diff : expr list -> expr list -> expr list
(* [diff l1 l2] returns [l1] without the formulas present in [l2] *)
val union : expr list -> expr list -> expr list
(* [union l1 l2] returns [l1 @@ l2], removing duplicate formulas *)
val disjoint : expr list -> expr list -> bool
(* [disjoint l1 l2] returns true if [l1] and [l2] have no element in common *)
