...
let rec print_var_all oc l = 
  match l with
  |[] -> ()
  |(Evar (x, _))::[] -> Printf.fprintf oc "%s" x
  |(Evar (x, _))::l' -> Printf.fprintf oc "%s, %a" x print_var_all l'
  |_ -> ();;
...
match e with
  ...
  |Evar (x, _) -> Printf.fprintf oc "%s" x
  |Eapp (e, [], _) -> Printf.fprintf oc "%a" print_expr e
  |Eapp (Evar("=", _), e1::e2::[], _) -> Printf.fprintf oc "%a = %a" print_expr e1 print_expr e2
  |Eapp (Evar("!=", _), e1::e2::[], _) -> Printf.fprintf oc "%a != %a" print_expr e1 print_expr e2
  |Eapp (e, l, _) -> Printf.fprintf oc "%a (%a)" print_expr e print_expr_list l
  |Eor (e1, e2, _) ->  Printf.fprintf oc "%a|%a" print_expr e1 print_expr e2
  |Eall (v, e, _) as f -> Printf.fprintf oc "![%a] : (%a)" print_var_all (get_var_all f)  print_expr (get_e_all f)
  |Eex (v, e, _) as f -> Printf.fprintf oc "?[%a] : (%a)" print_var_all (get_var_ex f)  print_expr (get_e_ex f)
  |Enot (e, _) -> Printf.fprintf oc "~%a" print_expr e
  |Eimply(a, b, _) -> Printf.fprintf oc "%a => %a" print_expr a print_expr b
  |Eequiv(a, b, _) -> Printf.fprintf oc "%a <=> %a" print_expr a print_expr b
  |_ -> failwith "Formula not accepted"
...
(* print predicate arguments P(x, ..., y) *)
  and print_expr_list oc l = 
    ...
    |[] -> ()
    |e::[] -> print_expr oc e
    |e::l' -> Printf.fprintf oc "%a, %a" print_expr e print_expr_list l'
  ;;
type definition =
