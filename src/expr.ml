...
(* get all quantified vars ![X, ..., Y] *)
let rec print_var_all oc l =
  match l with
  | [] -> ()
  | [Evar (x, _)] -> fprintf oc "%s" x
  | Evar (x, _) :: l' -> fprintf oc "%s, %a" x print_var_all l'
  | _ -> ()
...
(*let rec expr_to_string e =
  ...
  | Eall (v, e, _) as f ->
    "! [" ^ print_var_all (get_var_all f) ^ "] : ("
    ^ expr_to_string (get_e_all f) ^ ")"
  | Eex (v, e, _) as f ->
    "? [" ^ print_var_all (get_var_ex f) ^ "] : ("
    ^ expr_to_string (get_e_ex f) ^ ")"
...
let rec print_expr oc e =
  match e with
  | Efalse -> fprintf oc "$false"
  | Etrue -> fprintf oc "$true"
  | Evar (x, _) -> fprintf oc "%s" x
  | Eapp (e, [], _) -> fprintf oc "%a" print_expr e
  | Eapp (Evar("=", _), e1::e2::[], _) ->
...
and print_expr_list oc l =
  ...
  | [] -> ()
  | [e] -> print_expr oc e
  | e::l' -> fprintf oc "%a, %a" print_expr e print_expr_list l'
...
let rec diff l1 l2 =
  match l1 with
  | [] -> []
  | e::t -> if List.exists ((==) e) l2 then diff t l2 else e :: diff t l2
...
let rec disjoint l1 l2 =
  match l1 with
  | [] -> true
  | h::t -> if List.exists ((==) h) l2 then false else disjoint t l2
...
let list_var_equal l1 l2 =
  List.length l1 =%= List.length l2 && (List.for_all2 var_equal l1 l2)
...
let prop_app l =
    let aux l =
      try
        let x = List.find (fun e -> not (e == type_prop)) l in
        raise (Type_Mismatch (type_prop, x, "Expr.prop_app"))
      with Not_found -> type_prop
    in
    if List.memq type_iota l then type_iota
    else if List.memq type_none l then type_none
    else aux l

(* Meta-data constructors *)
(* Since expressions are hashconsed, actual constructors are defined a
   bit later *)
(* Additionally, the application needs more work and is defined
...
let rec print b ex =
  ...
  | Earrow (args, ret, _) ->
      bprintf b "(%a -> %a)"
        (fun b l ->
          List.iteri
            (fun i x -> if i > 0 then bprintf b " * "; bprintf b "%a" print x)
            l)
        args print ret;
  | Eapp (s, es, _) ->
      bprintf b "(%a%a)" print s
      (fun b l -> List.iter (fun x -> bprintf b " %a" print x) l) es
  | Enot (e, _) ->
      bprintf b "~ %a" print e
  | Eand (e1, e2, _) ->
      bprintf b "(%a /\\ %a)" print e1 print e2
  | Eor (e1, e2, _) ->
      bprintf b "(%a \\/ %a)" print e1 print e2
  | Eimply (e1, e2, _) ->
      bprintf b "(%a => %a)" print e1 print e2
  | Eequiv (e1, e2, _) ->
      bprintf b "(%a <=> %a)" print e1 print e2
  | Etrue -> bprintf b "True";
  | Efalse -> bprintf b "False";
  | Eall (v, e, _) ->
      bprintf b "(All.(%a : %a): %a)" print v print (get_type v) print e
  | Eex (v, e, _) ->
      bprintf b "(Exi.(%a : %a): %a)" print v print (get_type v) print e
  | Etau (v, e, _) ->
...
and eapp (s, args) =
  assert (is_var s); he_merge (Eapp (s, args, priv_app s args))

(* Delay the actual substitution of type arguments so that the
   complexity stays linear *)
and inst_app map s args = match s, args with
  | Eall(v, e, _), a :: r ->
     assert (get_type v == type_type);
     if get_type a == type_type then inst_app ((v, a) :: map) e r
     else raise (Type_Mismatch(type_type, get_type a, "Expr.inst_app"))
...
and substitute_unsafe map e =
  let aux f map v body =
    ...
    let nv =
      if conflict v map1 then tvar (newname ()) t
      else tvar (get_name v) t
    in
    if not (equal v nv) then f (nv, substitute_unsafe ((v, nv) :: map1) body)
    else f (v, substitute_unsafe map1 body)
  in
  match e with
  ...
  | Eapp (Evar ("=", _) as s, ([a; b] as args), _) ->
     begin
       try eapp (s, List.map (substitute_unsafe map) args)
...
let rec substitute_expr map e =
  ...
  | Emeta (e', _) -> e
  | Earrow(args, ret, _) ->
    earrow (List.map (substitute_expr map) args) (substitute_expr map ret)
...
let split_list n l = split_list_aux n l []
...
let get_tvar e = List.rev (get_tvar_aux [] e)
